    ...
    <div class="query" style="padding-left:30px">
      ...
      >
        ...
        <el-form-item label="重跑">
          <el-select
            v-model="queryForm.isFailedRetry"
            size="mini"
            style="width:100px"
          >
            <el-option
        ...
        <el-form-item label="展示依赖">
          <el-select
            v-model="queryForm.showDetail"
            size="mini"
            style="width:100px"
          >
            <el-option
              v-for="item in showDetailOptions"
              :key="item.label"
              size="mini"
              :label="item.label"
              :value="item.value"
        ...
        >
          <template slot-scope="scope">
            <el-tag
              ...
              disable-transitions
              >{{ scope.row.isFailedRetryValue }}</el-tag
            >
          </template>
        </el-table-column>
        <el-table-column
          property="isRelyCaseValue"
          label="用例依赖"
          min-width="8%"
        >
          <template slot-scope="scope">
            <el-tag
              ...
              disable-transitions
              >{{ scope.row.isRelyCaseValue }}</el-tag
            >
          </template>
        </el-table-column>
                ...
                <el-col :span="22">
                  <el-form-item label="body" label-width="100px">
                    <el-input
                      ...
                      type="textarea"
                      :autosize="{ minRows: 2, maxRows: 6 }"
                      v-show="!isReqBodyBeauty"
                    ></el-input>
                    <!-- <json-viewer :value="dataInfo.requestBody" :expand-depth=5 copyable v-show="isReqBodyBeauty"/> -->
                  </el-form-item>
                </el-col>
                <el-col :span="2">
                  <el-button
...
export default {
  ...
  methods: {
    ...
    async clickReqJson() {
      this.isReqBodyBeauty = !this.isReqBodyBeauty;
      if (this.JsonInfo.requestBody != null) {
        if (this.isReqBodyBeauty) {
          if (typeof this.JsonInfo.requestBody == "string") {
            this.JsonInfo.requestBody = JSON.parse(this.JsonInfo.requestBody);
          }
        } else {
          if (typeof this.JsonInfo.requestBody == "object") {
            this.JsonInfo.requestBody = JSON.stringify(
              this.JsonInfo.requestBody
            );
          }
        }
      }
